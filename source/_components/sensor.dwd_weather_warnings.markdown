...
| `warning_<x>_type` | *(int)* Issued warning type. <br/>0 == ?, <br/>1 == Windböen, Sturmböen, <br/>2 ==  ?, <br/>3 ==  Schneefall, <br/>4 ==  Nebel,<br/>5 ==  Frost, <br/>6 == Glätte or Glatteis<br/>Please be aware that the type numbers represent more like a category than an exact number-to-string match. For example Type `6` can mean `GLÄTTE` or `GLATTEIS` or similar. |
